...
/*
 ...
 - 횡단 관심사(Cross-Cutting Concern) : 핵심(core) 비즈니스 로직(삼겹살 구워먹기)
 					다소 거리가 있지만, 여러 모듈에서 공통적이고 반복적인 처리를 요구하는 내용
 					(불판닦기, 불판교체)
 - 횡단 관심사 분리(Separation of Cross-Cutting Concern) : 횡단 관심사에 해당하는 부분

 - Component : 골뱅이Aspect와 짝꿍. component-scan시 "여기 봐주세여" 라는 의미
 - JoinPoint : 어드바이스(특정 조인 포인트(불판이 탔을 때)에서 실행되는 코드. 횡단 관심사를 실제로 구현)
			      적용될 수 있는 위치
